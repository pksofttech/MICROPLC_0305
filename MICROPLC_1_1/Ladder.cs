...
namespace MICROPLC
{
	...
	static public class Ladder
	{
        ...
        public static void Switch_language(string language)
        {
            switch (language)
            {
                case "en":
                    ...
                    break;
                case "th":
                    cul = CultureInfo.CreateSpecificCulture("th");    //create culture for vietnamese
                    break;
                default:
                    cul = CultureInfo.CreateSpecificCulture("en");     //create culture for english
                    break;
			...
			try {
				foreach (ManagementObject item in searcher.Get()) {
					...
					if (desc.Contains("Arduino")) {
						Ladder.strCommunicationName = deviceId;
						(Application.OpenForms["MainForm"] as MainForm).SettoolStripPortname(desc);  
						return deviceId;
					}
		...
		public static string strCommunication_baud_rate = "115200";
		...
		static  Elements StrLadderToTag(ref string strladder, Elements parent)
		{
			...
			//*********************** START IMPORT FORM LDMICRO ***************************//
				...
				case "CTC":
                    ...
                    {
                        Properties_auto_clear = true,
                        Properties_value1 = 0,
                        Properties_value = int.Parse(strTemp[2])
                    };
                    return tag_Ctc;
...
//***************************************************************************
			...
			foreach (Elements temptag in VariablePLCLib_element) {
				switch (temptag.Type) {
					...
					case TypeTag.SHIFT_VARIABLE:	
						...
						break;
					case TypeTag.TOF:
						strGenIO += string.Format("\t\tTimerOff {0}({1});\t\t//{2}\n", temptag.Name, temptag.Properties_value, temptag.Type);									
						break;
					case TypeTag.TON:
						//strGenIO += string.Format("\t\tunsigned long {0};\t\t//{1}\n", temptag.Name, temptag.Type);								
						strGenIO += string.Format("\t\tTimerOn {0}({1});\t\t//{2}\n", temptag.Name, temptag.Properties_value, temptag.Type);									
						break;
					case TypeTag.CONTACTS:	
						strGenIO += string.Format("\t\tunsigned int {0};\t\t//{1}\n", temptag.Name, temptag.Type);								
						break;
					...
					case TypeTag.TPC:	
						...
						strGenIO += string.Format("\t\tTimerCycle {0}({1},{2});\t\t\n", temptag.Name, temptag.Properties_value, temptag.Properties_value1);									
		...
		public static String GenPLCLib_Code(ProgramLadder program)
		{
			...
			strGenCode += "\tscan_input_plc();\t//***Read Input***//\n\tscanTimeValue = millis();\n\tMasterscanValue = 1;\n";
			...
			try {
				switch (element.Type) {
					...
					case TypeTag.TOF:
						...
						Op_Code_Tag = string.Format("{0}.process();", element.Name, element.Properties_value);							
						break;
					case TypeTag.TON:
					...
					case TypeTag.TPC:
						...
						Op_Code_Tag = string.Format("{0}.process();", element.Name, element.Properties_value, element.Properties_value1);
