...
namespace MICROPLC
{
	...
	{   
		...
		Translator translator_str = new Translator();
		void Translator_content()
		{

		}
		public ProgramLadder program = Ladder.program;
		int tag_size	= DrawingTags.tag_draw_size;
		string TitleStr;
...
//			groupBox4.BackColor = groupBox3.BackColor;
			// SET DELEGETE
			//mTimer1 = new AccurateTimer(this, new Action(TimerTick1), 1000);
			...
			ts_arduino_Pro1685v.Click += SelectHardware;
			ts_arduino_NANO.Click += SelectHardware;
			ts_arduinoUNO.Click += SelectHardware;
			ts_arduinoUNOR3.Click += SelectHardware;
			ts_arduino_MEGA.Click += SelectHardware;
			ts_Micro_PLC_deca.Click += SelectHardware;
        ...
        private void Ladder_panelMouseMove(object sender, MouseEventArgs e)
		{
            switch (cursor_Mode)
            {
                case Cursor_Mode.draging:

                    //Text = rectangle_select.ToString() + "*******" + e.Location.ToString();					
		...
		void Ladder_panelMouseDown(object sender, MouseEventArgs e)
		{
			...
			if (me.Button == MouseButtons.Left & cursor_Mode == Cursor_Mode.select_mode) {				
				...
				rectangle_select.X = cursor_mouse.X;
				rectangle_select.Y = cursor_mouse.Y;				
			}
...
//						}
						program.SelectTags(rectangle_select);   
						//toolTip_element.Hide(Ladder_Draw);
						
						if (program.Select_tags.Tags.Count == 1) {
							toolTip_element.ToolTipTitle = program.Select_tags.Tags[0].Name;
							toolTip_element.Show(program.Select_tags.Tags[0].getInfo(), Ladder_Draw, program.select_area.X + program.select_area.Width, program.select_area.Y, 2000);
						} else {
							toolTip_element.Hide(Ladder_Draw);
						}
...
//							rectangle_select.Width	= Tag_Size * 2;	
//						}
						//Text  = rectangle_select.ToString();
						program.SelectTags(rectangle_select);    					
		...
		{	
			...
			//btn_element_Functions.Enabled = true;
			return 0;
		}
		string LdtoStrCode()
		{
		...
		void OpenToolStripMenuItemClick(object sender, EventArgs e)
		{
			if (Ladder.saveStratus) {
				DialogResult retdialog = MessageBox.Show("Do you want to save the work in progress?", "SAVE LADDER FILE", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Warning);
				switch (retdialog) {
					case DialogResult.Yes:
		...
		{
			try {
				string ladderstr = file;
				if (file.IndexOf("# LD FORMAT copy Form LDMICRO#", StringComparison.Ordinal) == -1)
					ladderstr = File.ReadAllText(file);
...
//			}
		...
		{
			String strElement = sender.ToString();
			...
			if (tagSelect != null)
				switch (Ladder.Hardware_name) {
					case "Micro PLC deca":
						...
						break;	
					case "Arduino UNO R3":
						arduino_unoMapIO mapIoArduinor3 = new arduino_unoMapIO(tagSelect);
						retMapIO = mapIoArduinor3.ShowDialog();
						if (retMapIO == DialogResult.Cancel)
							CreateListElement();
						break;
					...
					case "Arduino MEGA":
						MapIOPorts mapIoArduino = new MapIOPorts(tagSelect);
						retMapIO = mapIoArduino.ShowDialog();
						if (retMapIO == DialogResult.Cancel)
							CreateListElement();
						break;
					default :
						MessageBox.Show("Please select devices for use ...", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
						break;
				}
		}
			...
			if (tagSelect != null)
				switch (Ladder.Hardware_name) {
					case "Micro PLC deca":
						...
						break;	
					case "Arduino UNO R3":
						arduino_unoMapIO mapIoArduino3 = new arduino_unoMapIO(tagSelect);
						retMapIO = mapIoArduino3.ShowDialog();	
						if (retMapIO == DialogResult.Cancel)
							CreateListElement();
						break;
					case "Arduino ProMini(168)5V":
        ...
        private string GetGen_Code()
        {
            ...
            string str_Variable = "";
            string ladderstr = Regex.Replace(LdtoStrCode(), @"\n|\r", @"\n");
            ladderstr = Regex.Replace(ladderstr, @"\t", @"\t");
            foreach (Elements tempVar in Ladder.VariablePLCLib_element)
            {
                switch (tempVar.Type)
                {
                    case TypeTag.CONTACTS:
                        ...
                        str_Input += string.Format("\tinplc{2}({0},_{1});\n", tempVar.Name, tempVar.IO_Port, strInputMode);
                    ...
                    case TypeTag.COIL:
                        if (tempVar.Name.Substring(0, 1) == "Y")
                        {
                            ...
                            str_Output += string.Format("\toutplc({0},_{1});\n", tempVar.Name, tempVar.IO_Port);
                            strConfigPin += string.Format("\tpinMode(_{0},OUTPUT);\n", tempVar.IO_Port);
                        }
                        break;
                }
                str_Variable += string.Format("\tcreatValueMsg({0});", tempVar.Name) + "\n";
            }
            ...
            string LoopPlc_Code = "\t\tloop_plc();";
            string scanPlcisr = Ladder.GenPLCLib_Code(program);
            ...
            + "\tTimer1.initialize(" + Ladder.cycle_loop_value * 1000 + ");\n\tTimer1.attachInterrupt( scanPlcIsr ); // attach the service routine here\n}\n"

            + "void scan_input_plc() {\n" + str_Input + "}\n"
            + "void scan_output_plc() {\n" + str_Output + "}\n"
            + "void scanPlcVariable() {\n\tclear_Msg();\n" + str_Variable + "\tsendValueMsg();}\n"
            + "void scanPlcIsr() {\n" + scanPlcisr + "\nscanPlcVariable();\n}\n"
            + "void _print_ldcode() {\n\t\tSerial.print(F(\"" + ladderstr + "\"));\n}\n"
            + "\tvoid loop() \n\t{\n\t" + LoopPlc_Code + "\n\t}\n\t//#####################################################//";
            return strCodeGen;
        }

        int Gen_compile_mcu_code()
		{
			switch (Ladder.Hardware_name) {
				case "Arduino UNO R3":
				case "Arduino/NANO UNO(328)":
		...
		void Export_CodeClick(object sender, EventArgs e)
		{
			...
			int compileCode = Gen_compile_mcu_code();		
			if (compileCode == -1)
				return;
		...
		void Btn_simulationClick(object sender, EventArgs e)
		{
			...
			if (DrawingTags.simulation_startus) {
				...
				t_lable_Simulation_time.Enabled = true;
				SetupSimulation();
				ts_bnt_start.Enabled = true;
				ts_bnt_stop.Enabled = false;
				...
				t_lable_Simulation_time.Enabled = false;
		...
		void StartSimulate(object sender, EventArgs e)
		{
			...
			//mTimer1 = null;
			mTimer1 = new AccurateTimer(this, new Action(TimerTick1), 100);
			ts_bnt_start.Enabled = false;
			ts_bnt_stop.Enabled = true;
			simulation_time = 0;
		}
		...
		*/
            ...
            DrawingTags.simulationTime += (ulong)(100 * mup);
...
//					continue;
		...
		void SelectHardware(object sender, EventArgs e)
		{
			...
			if (Ladder.Hardware_name == "Micro PLC deca") {
				//ts_bnt_connect.Enabled = true;
			} else {
				//ts_bnt_connect.Enabled = false;
			}
		...
		{
			var option_Config = new Option_Config();
		...
		void Ts_bnt_connectClick(object sender, EventArgs e)
		{
			string comport_name;
			//textBox_plc_info.Text = "";
			if (Ladder.strCommunicationName.IndexOf("COM", StringComparison.Ordinal) > -1) {
				comport_name = Ladder.strCommunicationName;
			} else {
				
				comport_name = Ladder.AutodetectArduinoPort();
				if (comport_name == "") {					
					...
					Ladder.strCommunicationName = comport_name;
				}
			...
			if (serialPort1.IsOpen)
				serialPort1.Close();
			serialPort1.PortName = comport_name;			
			serialPort1.Open();
			
			if (serialPort1.IsOpen) {
				wait_msg = false;
				serialPort1.WriteLine("chks");
			}
		...
		void SerialPort1DataReceived(object sender, System.IO.Ports.SerialDataReceivedEventArgs e)
		{
			...
			foreach (byte b in comBuffer) {
				Process_byte(b);
			}		
			
		}
		string comMode = "ready";
		string msgData = "";
				...
				case "ready":
					switch (b) {
						...
						case 0x27:
							...
							break;
						default:
								//textBox_plc_info.Text += string.Format("{0},", b);
							break;
					}                  			
					break;
				case "Text":
					switch (b) {
						...
						case 0x0c:
							Process_msg("#text#" + msgData);
							msgData = "";
							comMode = "ready";
							break;
						default:
							msgData += Convert.ToChar(b);
							break;
					}  
					break;
				case "command":							
		...
		bool wait_msg = false;
		void Process_msg(string  str_msg)
		{
			if (wait_msg)
				return;
			BeginInvoke(new EventHandler(delegate {
			     
				if (str_msg.IndexOf("#text#", StringComparison.Ordinal) == 0) {
					str_msg = str_msg.Substring(6).Trim();
					if (str_msg.IndexOf("# LD FORMAT copy Form LDMICRO#", StringComparison.Ordinal) == 0) {
						MessageBox.Show(str_msg, "code");
						
						SelectHardware(ts_no_hw, null);
						ImportFormFile(str_msg);
						Text = string.Format("{0} - {1}", TitleStr, "Ladder In PLC");
						Ladder.SaveStratus = false;
						Ladder_Draw.Invalidate();
						CreateListElement();	
						ts_Hardware_name.Text = Ladder.Hardware_name;
						...
						//textBox_plc_info.Text = str_msg;	
					}
					
				}
				if (str_msg.IndexOf("#command#", StringComparison.Ordinal) == 0) {
					//textBox1.Text = str_msg.Substring(9);
					if (DrawingTags.sync_startus)
						Sync_step(str_msg.Substring(9));
				}
				
			}));
		}
		int count_msg = 0;
		void Sync_step(string str_variable)
		{
			count_msg++;
			if (count_msg > 90)
			if (serialPort1.IsOpen) {
				serialPort1.WriteLine("clear_count_msg");
				count_msg = 0;
			}
			str_variable = str_variable.Trim();
			string[] s_varMsg = str_variable.Split(' ');
			if (Ladder.VariablePLCLib_element.Count == s_varMsg.Length) {
				int i = 0;				
				foreach (Elements tagVariable in Ladder.VariablePLCLib_element) {
					int x = 0;
					switch (tagVariable.Type) {
						case TypeTag.COIL:
						case TypeTag.CONTACTS:
							
							Int32.TryParse(s_varMsg[i], out x);
							tagVariable.Startus = Convert.ToBoolean(x);
							tagVariable._ct = x;
							break;
						case TypeTag.TOF:
						case TypeTag.TON:
				...
				wait_msg = true;
				MessageBox.Show(Ladder.VariablePLCLib_element.Count.ToString() + "---" + s_varMsg.Length + str_variable, "Can,n Sync....PLC", MessageBoxButtons.OK, MessageBoxIcon.Stop);
			}
			
		}
		//		void Ts_bnt_Sync_modeClick(object sender, EventArgs e)
		//		{
		//			if (ts_bnt_Sync_mode.Text == "Sync...On") {
		//				ts_bnt_Sync_mode.Text = "Sync...Off";
		//				serialPort1.WriteLine("clear_count_msg");
		//				count_msg = 0;
		//				DrawingTags.sync_startus = true;
		//			} else {
		//				ts_bnt_Sync_mode.Text = "Sync...On";
		//				DrawingTags.sync_startus = false;
		//			}
		//		}
		void Button_resetClick(object sender, EventArgs e)
		{
			// Shortened and error checking removed for brevity...
			if (!serialPort1.IsOpen)
				return;
			if (serialPort1.IsOpen) {
				wait_msg = false;
				serialPort1.WriteLine("_read_strldcode");
				return;
			}
